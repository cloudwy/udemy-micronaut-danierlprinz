...
import static com.wy.udemy.broker.wallet.WalletController.SUPPORTED_FIAT_CURRENCIES;
...
public class FiatCurrencyNotSupportedExceptionHandler implements ExceptionHandler<FiatCurrencyNotSupportedException, HttpResponse<CustomError>> {
    ...
    public HttpResponse<CustomError> handle(HttpRequest request, FiatCurrencyNotSupportedException exception){
        CustomError customError = new CustomError(HttpStatus.BAD_REQUEST.getCode(), "UNSUPPORTED_FIAT_CURRENCY", String.format("Only %s are supported", SUPPORTED_FIAT_CURRENCIES));
        return HttpResponse.badRequest(customError);
    }
